...
class NgSpiceWrapper(object):
    ...
    def _simulate(self, fpath):
        ...
        # Run ngspice with the circuit file to execute control blocks
        command = ['ngspice', fpath]
    ...
    def _parse_result(self, design_folder):
        """
        Parses NgSpice simulation results from output files generated by control blocks.
        Looks for files like ac_out.txt, tran_vin.txt, tran_vout.txt, etc.
        """
        # With control blocks, ngspice generates specific output files in the simulation directory
        # Use the NgSpiceParser to parse the folder contents
        res = NgSpiceParser.parse(design_folder)
        return res
